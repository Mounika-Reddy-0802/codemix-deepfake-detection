...
| 2026-09-06 | W9-T2 | M | week9/mounika-publication-figures | publication figures built from committed measurements only: system x condition heatmap, det curves computed from the 71,237 per-clip scores rather than summaries, and the shortcut gate against chance; a test asserts the english column equals the retention json, and the three unmeasured artefacts are reported as gaps instead of drawn |
| 2026-09-06 | W4-T1-recovery | SK | week9-krishna-cm01-recovery | cm01 recovered off one laptop into git: 4,000-record portable log committed, 842 mb archived to a private kaggle dataset and verified 4000/4000 via a new clips-only mode, and pitch re-measured over all 4,000 clips at 18.99 hz which overturns p-021's 23-clip figure (p-023) |
| 2026-09-06 | W4-T3 | SK | week9-krishna-cm01-recovery | cm04 held-out tortoise staged: 500-job table over 15 eval-pool speakers committed before any gpu is booked, zero train-pool leakage, pool_jobs given a --tool flag rather than a parallel builder, kaggle notebook written; generation still outstanding |
| 2026-09-11 | W8-T1 | M | week10-mounika-unseen-attack-scoring | cm02 and cm04 scored on all three checkpoints: the clean adapter catches unseen tortoise at 5.03 pct eer (4x below its 22.21 pct shortcut floor) but misses rvc at 31.90 pct, waving 83 pct of conversions through as real; recorded as p-025 |
| 2026-09-11 | W9-T1 | M | week10-mounika-unseen-attack-scoring | bundle and attack sets level-normalised to -23 dbfs and the shortcut gate re-run: 5.17 pct clean, 10.01 pct channel, still failing on recording domain rather than level; speaker-held-out rvc split built and four retrain configs plus a kaggle notebook staged for the freeze (p-026) |
| 2026-09-13 | W9-T1 | M | week10-mounika-unseen-attack-scoring | four normalised adapters scored against floors measured per set: adding rvc cuts rvc eer to 5.01 pct but raises unseen tortoise from 5.44 to 12.01 pct clean, and both channel adapters sit above the new 25.79 pct cm04 channel floor; ablations.json, ablation figure and four new floors committed (p-027) |
| 2026-09-13 | W5-T3 | M | week10-mounika-unseen-attack-scoring | s3 native training staged: four configs mirroring the w10 adapters from the pretrained encoder, a config guard that checks every real training manifest against the dataset rule, a kaggle notebook that trains s3 and scores it plus the s2 adapters on english; cpu smoke train passes (p-028) |
