...
| 2026-09-13 | W5-T5 / W8-T4 | SK | week10-krishna-live-inference | live-call detection path built from the week-1 placeholders: 4 s / 2 s streaming with a silence gate and -23 dbfs normalisation, a verdict state machine with beep then sms escalation and a call summary, file prediction, and file replay through the shared pcm contract; a real-audio check showed the demo threshold must come from window-level scores (p-029) |
