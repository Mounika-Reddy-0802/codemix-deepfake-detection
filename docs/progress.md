...
| 2026-08-25 | W5-T4 | M | week7-lora-gap-closure | fixed the normalisation bug: portable_bundle.build now rms-normalises to -23 dbfs, class level gap 3.06 db -> 0.08 db; also fixed data_root being accepted and ignored there, which blocked rebuilding from a portable manifest |
| 2026-08-25 | W5-T4 | M | week7-lora-gap-closure | gate after the fix: 1.39% -> 5.17% eer, still failing; zcr is now the dominant cue (mucs 1.8x xtts), which is lecture audio vs vocoder and no gain change fixes it |
| 2026-08-25 | W8-T1 | M | week7-lora-gap-closure | s2 is not using the loudness artifact: retrained on normalised audio it scores the same 1.34%, and the old checkpoint scores 1.16% on normalised eval; margin over the shortcut goes 0x -> 3.9x |
